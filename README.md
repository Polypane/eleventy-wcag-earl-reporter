...
The expected format of this EARL file is somewhat different from other testing tools. Since most testing tools to not test all pages, they select a subset of pages and display issues per page or page type. With automated testing, we can scan all pages on a site individually.

Since there can be significantly more pages than unique issues, and since most issues are replicated across pages, the format of this EARL file lists issues and the pages they occur on, instead of pages and the issues they contain.
